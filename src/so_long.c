...
/*   Updated: 2024/09/29 18:54:00 by mfleury          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	cat->h_idle_m = create_anime(0.5, 0, 0);
	cat->h_run = create_anime(0, 0, 0);
	cat->h_dead = create_anime(0.5, 0, 0);
	if (cat->h_idle_m == NULL || cat->h_run == NULL || cat->h_dead == NULL)
		unexpected_close(ERR_ANIME, sl, sl->map);
	load_static_image(sl, cat);
	load_dynamic_image(*sl, cat->h_idle_m, cat->h_idle_sm);
...
static void	texture_load_sequence(t_win *sl, t_cat *cat)
{
	...
	cat->wall = load_texture(*sl->mlx, mirror(mlx_load_png(WALL)), g_wall);
